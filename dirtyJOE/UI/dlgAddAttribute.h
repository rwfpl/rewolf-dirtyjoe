...
class CDlgAddAttribute : public CDialogImpl<CDlgAddAttribute>
{
public:

	enum { IDD = DJ_ADD_CP };

	BEGIN_MSG_MAP(CDlgAddAttribute)
	...
	CDlgAddAttribute(CDirtyJOE& _dj, std::vector<attribute_info*>& _aiv) : dj(_dj), aiv(_aiv) {} 

private:

	CDirtyJOE& dj;
	std::map<u2, u2> addAttrMap;
	std::vector<attribute_info*>& aiv;

	bool checkSupport(JavaAttribute attrID);
};
