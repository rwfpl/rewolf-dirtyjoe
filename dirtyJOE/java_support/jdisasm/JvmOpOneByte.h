...
class CJvmOpOneByte : public IJvmOpcode, public CDynamicStructWithCP
{
	private:
		const _opcode_struct& opcode;

	public:
		CJvmOpOneByte(u1* mem, u4 maxLen, const JavaConstPool& cp, CLogger& log);
		virtual ~CJvmOpOneByte();
